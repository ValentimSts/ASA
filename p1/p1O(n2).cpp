    ...
    for(int i = 0; i < size; i++) {
        for(int j = 0; j < i; j++) {
            ...
            if(values[i] > values[j]) {
                ...
                // if the length of the LIS ending on index i (lengths[i]) is equal to lengths[j]
                // (length of an LIS without values[i]) it means that including values[i] in the
                // LIS ending on index j wouldn't increase its sice but rather result in a different
...
void LCIS(std::vector<int> &values1, std::vector<int> &values2, int len1, int len2)
{
    ...
    // finds the biggest vector to know how big the lcis vector should be.
    if(len1 > len2)
        size = len1;
    else
        size = len2;
    ...
    // finds the length of the longest commonincreasing subsequence.
    for(int len: lcis) {
        if(len > max_len)
            max_len = len;
    }
